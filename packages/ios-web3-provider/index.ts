...
function setConfig(config: IEthereumProviderConfig) {

  const strategy = 'CALLBACK';

  try {
    const core = window.trustwallet.core(strategy, (params: any) => {
      // Disabled methods
    ...
    // Generate instances
    const ethereum = window.trustwallet.ethereum(config);
    ...
    // Attach to window
    window.trustwallet.ethereum = ethereum;
    ...
    Object.assign(window.trustwallet, {
      ...
      on: (...params: any) => ethereum.on(...params),
      off: (...params: any) => ethereum.off(...params),
      enable: () => { return true }
    });
