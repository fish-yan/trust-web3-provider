...
import { BaseProvider, Web3Provider } from '@trustwallet/web3-provider-core';
...
var registerProviders: BaseProvider[] = []
...
function setConfig(config: IWalletConfig) {
  ...
  try {
    ...
    setEthereumConfig(config.ethereum)
    setSolanaConfig(config.solana)
    setAptosConfig(config.aptos)
...
function setEthereumConfig(config?: IEthereumProviderConfig) {
  const ethereum = new EthereumProvider(config);
  ethereum.providers = [ethereum];
  window.ethereum = ethereum;
  window.walletOnto.ethereum = ethereum;
  window.onto = ethereum
  ...
  registerProviders.push(ethereum)
}

function setSolanaConfig(config?: ISolanaProviderConfig) {
  const solana = new SolanaProvider(config);
  window.phantom = {
    solana: solana
  }
  window.solana = solana
  window.walletOnto.solana = solana

  registerProviders.push(solana)
}

function setAptosConfig(config?: IAptosProviderConfig) {
  const aptos = new AptosProvider(config);

  window.walletOnto.aptos = aptos

  registerProviders.push(aptos)
}

function setTonConfig(config?: ITonProviderConfig) {
  const ton = new TonProvider(config);
  const bridge = new TonBridge(ton);

  window.tonkeeper = {
    tonconnect: bridge
  }
  window.walletOnto.ton = ton
  window.walletOnto.tonconnect = bridge

  registerProviders.push(ton)
}

function setNeoConfig(config?: INeoProviderConfig) {
  const neo = new NeoProvider(config)

  window.neo = neo;
  window.OneGate = neo;
  window.Vital = neo;
  window.walletOnto.neo = neo;

  registerProviders.push(neo)
}

function setTronConfig(config?: ITronProviderConfig) {
  const tron = new TronProvider(config)
